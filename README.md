...
| | 값 |
|---|---|
| Primary | Indigo `#5B5FE6` |
| 시맨틱 | Delivered Emerald `#10B981` · In-transit Amber `#F59E0B` · Pending Slate `#94A3B8` · Failure Rose `#EF4444` |
| Typography | Pretendard (현재 시스템 폰트 fallback, ttf 추가 시 `core/designsystem/theme/Type.kt` 한 줄 교체) |
| Shape | 카드 16dp · 큰 컨테이너 24dp · pill 캡슐 |
| Elevation | 0 (그림자 대신 1dp `outlineVariant` 보더로 분리) |
| 다크 모드 | 자동, Dynamic Color는 Android 12+ |
| 시스템 바 | edge-to-edge, 투명 |

3개 화면 모두 디자인 mockup → Compose 재현 사이클을 거쳤습니다.
...
### 클론 후 처음 한 번
1. `git clone git@github.com:alsrbs12304/TrackBoxV2.git`
2. Android Studio에서 프로젝트 열기 → 자동 Gradle sync
3. `local.properties`는 자동 생성 (Android SDK 경로)
4. `gradle.properties` 의 `org.gradle.java.home` 라인을 본인 머신 JDK 17 경로로 수정하거나 삭제
...
adb shell am start -n com.mgpark.trackbox/.MainActivity
```
